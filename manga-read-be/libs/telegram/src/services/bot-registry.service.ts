...
import { OnUpdateMetadata } from '../types';
import {
...
export class DefaultBotRegistryService implements OnModuleInit {
  ...
  public async onModuleInit() {
    const discoveredClassesWithMeta =
  ...
  private registerOnHandlers(discoveredClass: DiscoveredClass) {
    const discoveredMethodsWithMeta =
      this.discoveryService.classMethodsWithMetaAtKey<OnUpdateMetadata>(
        discoveredClass,
        ON_UPDATE_METADATA_KEY,
      );
    ...
    for (const discoveredMethodWithMeta of discoveredMethodsWithMeta) {
      ...
      this.bot.on(
        trigger,
        (context) => {
          return discoveredMethod.handler.bind(
